type TSDA    #10.7 slide 18
  ...
  function TSDA(sim::Sim,bodyi::Body,bodyj::Body,Pi::Int,Qj::Int,k=0,l₀ = 0,c = 0,h = (lij,lijdot,t)->0 )
    new(sim,bodyi,bodyj,Pi,Qj,k,l₀,c,h)
  end
end
...
Fi(tsda::TSDA) = f(tsda)[1]*eij(tsda)  #extract scalar float from matrix
Fj(tsda::TSDA) = f(tsda)[1]*-eij(tsda)
...
nbarj(tsda::TSDA) = tilde(pt(tsda.bodyj,tsda.Qj))*A(tsda.bodyj)*Fj(tsda)
